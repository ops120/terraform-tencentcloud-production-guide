...
> 🎯 **如果你是零基础**：学完 **Part A（网络 + 计算）** 就可以直接跳到 Phase 4，Part B 等需要时再回来学。
...
> - [ ] ✅ 安全组规则生效
>
> > 🎯 **如果你已经能完成以上 4 项，说明网络 + 计算的基础已经掌握，可以选择跳过 Part B 直接进入 Phase 4**
>
> 以下开始 **Part B：数据库 + 存储 + 负载均衡**

> 📌 **以下内容为选学**：当你需要数据库、负载均衡、存储等高级资源时再回来学习。

---

## 🔧 选学 3.3 数据库

### 🔧 选学 3.3.1 TencentDB for MySQL
...
完成以下要点就说明你已掌握 Part A 的基础内容：

- [ ] **3.1 VPC 网络**：能创建 VPC、公网/内网子网、路由表、NAT 网关
- [ ] **3.2 计算资源**：能创建 CVM 实例、配置密钥对、通过 SSH 登录
- [ ] ✅ 能独立完成 `terraform apply` → SSH 登录验证 → `terraform destroy` 的完整流程
- [ ] ✅ 理解公网子网和内网子网的区别与用途

> 以上 4 项确认后，即可进入 **Phase 4** 继续学习。Part B 的内容在你需要时再回来参考即可。
