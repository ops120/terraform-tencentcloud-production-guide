...
variable "admin_ip" {
  description = "管理 IP（用于 SSH 访问）"
  type        = string
  default     = "0.0.0.0/0"  # 建议改为你的公网 IP，如 "123.123.123.123/32"
}
...
│  ┌──────────────────────────────────────────┐    │
│  │           CLB 负载均衡层                   │    │
│  │  (HTTPS:443 / HTTP:80 / WebSocket)       │    │
│  └────────────────┬─────────────────────────┘    │
│                   │                              │
│  ┌────────────────▼─────────────────────────┐    │
│  │         Web 计算层 (Auto Scaling)         │    │
│  │  [CVM-01] [CVM-02] [CVM-03] [CVM-04]   │    │
│  │  启动配置 + 伸缩策略 + 健康检查           │    │
│  └────────────────┬─────────────────────────┘    │
│                   │                              │
│  ┌────────────────▼─────────────────────────┐    │
...
如果业务需要多环境网络隔离（如生产 VPC 与测试 VPC 隔离），可以通过 VPC 对等连接实现互通。以下示例假设已创建 `tencentcloud_route_table.prod` 和 `tencentcloud_route_table.test`（参见 3.1 节）：
...
> - 同账号同地域对等连接会自动接受，但 Terraform 中仍需 `accept_operation` 资源完成状态变更
...
```
第 1 周  │ Phase 1 基础入门 + Phase 2 腾讯云入门
         │ 目标：能创建单个 CVM 实例
         │ 完成项目一
         │
...
**答案**：跨可用区部署可以避免单点故障。如果一个可用区发生故障（如电力中断），其他可用区的 CVM 继续提供服务。CLB 会自动将流量分发到健康的实例上。
</details>
